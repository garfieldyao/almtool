/*
 * $Id: SnmpOperator.java, 2011-11-13 ����7:03:42 Yao Exp $
 * 
...
public class SnmpOperator {
    ...
    private CommunityTarget getSnmpSession() {
        ...
        target.setAddress(GenericAddress.parse("udp:" + ne.getIpAddress() + "/" + ne.getPort()));
    ...
    public VariableBinding getSingleValue(String oid, int timeout) {
        ...
        try {
            ...
            if (response == null)
                throw new SnmpException(ne.getIpAddress(), SnmpException.SNMP_TIME_OUT);
            if (response.getErrorStatus() > 0)
                throw new SnmpException(ne.getIpAddress(), SnmpException.SNMP_GET_ERROR);
    ...
    public VariableBinding[] getValues(String[] oids, int timeout) {
        ...
        if (len <= MAX_OID_NUM) {
            ...
            String[][] tmpoids = remainder == 0 ? new String[cnum][] : new String[cnum + 1][];
    ...
    public void setSingleValue(String oid, Variable var) {
        ...
        try {
            ...
            if (response == null)
                throw new SnmpException(ne.getIpAddress(), SnmpException.SNMP_TIME_OUT);
            if (response.getErrorStatus() > 0)
                throw new SnmpException(ne.getIpAddress(), SnmpException.SNMP_SET_ERROR);
        } catch (IOException ex) {
            logger.error(ne.getIpAddress(), ex);
        } finally {
            if (snmp != null) {
    ...
    public void setValues(String[] oids, Variable[] vars) {
        if (ArrayUtils.isEmpty(oids) || ArrayUtils.isEmpty(vars) || oids.length != vars.length)
            return;
        CommunityTarget session = getSnmpSession();
        DefaultUdpTransportMapping transport = null;
        Snmp snmp = null;
    ...
    public VariableBinding[] getTableColumnValues(String oid) {
        ...
        int MaxRepetitions = ClientContext.getMaxRepetitions();
        List<VariableBinding> result = new ArrayList<VariableBinding>();
    ...
    public VariableBinding[] getColumnByIndex(List<String> indexes, String oid, int timeout) {
    ...
    public VariableBinding[][] getColumnsByIndex(List<String> indexes, String... columnOids) {
